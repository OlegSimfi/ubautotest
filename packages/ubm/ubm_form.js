...
function validateInput(aID, formCode, formEntity){
    if (!App.domain.byName(formEntity)){
    ...
    var cachedData = resultDataCache;
    var theSameCode = LocalDataStore.doFilterAndSort(cachedData, {whereList: {
...
function doUpdateInsert(ctxt, storedValue, isInsert){
    ...
    if (!docBody){
        if (storedValue.formType === 'auto') {
            ...
            var codeParts = storedValue.code.split('-');
            var className = formEntity.modelName + '.' + (codeParts[1] ? codeParts[1] : 'BetterToSetFormCodeToEntity-ClassName');
