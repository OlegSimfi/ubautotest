   ...
   // weight: 100,
    ...
    createItems: function(){
        ...
        grid.on('boxready',function(){
            var items = [], columns = [grid.columns.length];
    ...
    dataBind: function(){
        var me = this,
            grid = me.grid,
            request, fields = [],
            filterWhereList, idCountExist = false;
        ...
        $App.connection.select(request).done(function(response){
            ...
            me.items.each(function(item){
                ...
                if (item.baseColumn.summaryType){
                    ...
                    el = item.getEl();
                    el.setHTML(
                        item.baseColumn.summaryType === 'count' ? Ext.util.Format.numberRenderer('0,000')(data) :
                        (item.baseColumn.renderer ? item.baseColumn.renderer(data): data)
                    );
                    if (!item.toolTipe){
